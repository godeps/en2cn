...
如需进一步定制或接入生产环境，建议先运行生成脚本获得完整数据，再根据业务要求（词库、权重、过滤规则等）调整相关 JSON 或算法。

## 8. 自动化纠错方案梳理

为让“常用英文词”更贴近日常读法，可结合以下机制建立自动化闭环：

1. **词频驱动的校对清单**：定期统计线上/语料中的英文词频，批量跑转换结果，再与品牌官方译名、百科词条或人工维护的黄金表对比，自动生成“疑似错误”列表并导入 `manual_overrides`。
2. **ASR 回流验证**：将 TTS 播报后的音频送入 ASR，检查识别出来的中文是否匹配目标词；偏差大的词自动加入人工审核或直接追加 override。
3. **多候选 PK**：对得分较低或差异大的词保留前 K 个候选，结合语言模型、词频、词性等特征做二次排序；若最高分仍低于阈值，进入待校正队列。
4. **外部语料增补**：解析新闻稿、字幕、百科等中英文对照资源，抓取常见音译词并自动同步到 override 表或词库中。
5. **规则+模型融合**：沉淀常见后缀/结构的音译规则（如 "-tion"→"逊"），用规则或小模型纠偏自动得分较低的条目。
6. **灰度与反馈**：对不同候选做灰度实验或采集用户反馈，将“被纠正/被投诉”的样本自动打回训练与 override 管道，形成闭环。

结合这些策略，系统可以在“自动匹配 → 检测风险 → 建议修正 → 发布”的循环中不断提升准确率，同时仍保留 `manual_overrides` 做即时兜底。
